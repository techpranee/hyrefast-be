...
- Return ONLY the JSON array, no additional text`;
  ...
  async generateWithOllama(prompt) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `Attempt ${attempt}/${maxRetries}: Generating interview questions with Ollama`
        );
        ...
        if (error.response?.status === 404) {
          throw new Error(
            `Ollama service not found at ${this.ollamaApiUrl}. Please ensure Ollama is running and accessible.`
          );
        }
    ...
    if (lastError.code === "ECONNABORTED") {
      throw new Error(
        "Ollama request timeout - please check if Ollama is running and accessible"
      );
    }
    if (lastError.code === "ECONNREFUSED") {
      throw new Error(
        `Cannot connect to Ollama at ${this.ollamaApiUrl} - is it running and accessible?`
      );
    }
    if (lastError.message?.includes("model")) {
      throw new Error(
        `Ollama model '${this.ollamaModel}' not found - please pull the model first using: ollama pull ${this.ollamaModel}`
      );
    }

    throw new Error(
      `Interview questions generation failed after ${maxRetries} attempts: ${lastError.message}`
    );
  }
  ...
  getSuggestion(error) {
    ...
    const suggestions = {
      ...
      response_generation:
        "The AI service returned an empty response. Try again or check the model availability.",
      json_parsing:
        "AI response could not be parsed as JSON. Using fallback questions.",
  ...
  async testConnection() {
    try {
      console.log(
        "Testing Ollama connection for interview questions service..."
      );

      const response = await this.generateWithOllama(
        'Generate one interview question for a Software Engineer position. Return as JSON: [{"text": "question", "timeLimit": 120, "allowRetry": false, "tags": ["Technical"]}]'
      );
