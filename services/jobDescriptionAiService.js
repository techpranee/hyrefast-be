...
Return only the job description text, no additional formatting or explanations.`;
  ...
  async generateWithOllama(prompt) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `Attempt ${attempt}/${maxRetries}: Generating job description with Ollama`
        );
    ...
    throw new Error(
      `Job description generation failed after ${maxRetries} attempts: ${lastError.message}`
    );
  }
  ...
  async testConnection() {
    try {
      ...
      const response = await this.generateWithOllama(
        "Generate a brief description for 'Software Engineer' position in one sentence."
      );
